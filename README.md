...
程序内置了一份 LPR 历史数据，并会优先读取本地缓存。点击“更新LPR”时，程序会优先从中国货币网公开接口获取数据，不需要 Token。
...
如公开数据源暂时不可用，也可以在软件中点击“设置Token”，填入并保存 Tushare Token，作为备用数据源。
...
然后在软件中点击“更新LPR”。如果公开数据源和备用 Tushare 都不可用，程序会继续使用本地缓存；缓存不存在时使用内置数据。
